...
body.push(P([B("On-hand quantity is now tracked. "), T("Each item has an "), B("On hand"), T(" count. When all units of an item are already out on rentals that overlap a customer's dates, the app stops offering it for those dates ("), B("“Out for these dates”"), T(") — so you can't promise a ball mount that's already gone. The editor turns the count "), B("amber"), T(" if you drop below one per trailer; keeping roughly one of each common attachment per trailer means any customer can be set up even when several trailers are out at once.")]));
body.push(P([B("Load rating matters. "), T("The truck-side parts — the "), B("hitch, ball mount, and ball"), T(" — must each be "), B("rated for the loaded trailer weight"), T(" (e.g. a 2-5/16\" ball and a Class IV+ mount for the 14K trailers). The booking page reminds customers of this, and each equipment type carries a "), B("“You'll need to tow this”"), T(" note you should keep accurate. Stock only add-ons rated for the trailers you pair them with.")]));
